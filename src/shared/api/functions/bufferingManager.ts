...
import { buildApiUrl, fetchFromGitHubApi } from './gitHubApi';
import { processRepository } from './repositoryProcessor';
...
export const ensureBufferHasData = async (): Promise<void> => {
  ...
  if (needsData()) {
    const { currentApiPage } = getBufferState();
...
export const getItemsFromBuffer = async (): Promise<{
  ...
  const { repositoryBuffer, hasMoreApiData } = getBufferState();
